...
Prometheus 的搭建方式有很多中，本文档中展示的是以docker方式搭建独立的prometheus服务器端。

*注：将其运行为k8s环境中的prometheus deployment可能是更常见的使用方式。*
...
2. 编写prometheus.yaml
   ...
   这里，我们仅配置了metrics收集，并不使用alert等配置项。
    ...
    我们需要按照如下方式添加需要收集metrics的target。 这里的10.xx.yy.zz:30080即为CIS-C deployment暴露的prometheus metrics监听端口。
    ```
    - job_name: "f5-kic-1-18"
        static_configs:
        - targets: ["10.xx.yy.zz:30080"]
...
目前CIS-C收集业务下发过程中各个关键节点的时耗数据，收集方式如下：
...
* "request_id":
  ...
  当前业务下发请求的ID，请求到来后，CIS-C内部会根据操作不同，将其拆解为不同的类型，不同的步骤，此ID可以在后续处理步骤中标识此处理步骤来自哪个请求，便于累加下发总耗时。

  CIS-C会对每个k8s事件分配独立的request_id, k8s事件包括 configmap/endpoints/service/.. 资源的add、update、delete。

* "kind": 
  业务下发后在CIS-C内部的处理类型，包括
  * 网络处理类：arp, fdb, 
  * k8s集群资源类：configmap, endpoint, node, service
...
性能测试过程中，我们使用ansible 脚本逐个创建多个资源，如以下代码所示，逐个创建100个资源。
...
图中可以看到耗时分布均在2秒以下，以09:48为分隔，分左右两部分，左侧为deploy耗时，右侧为delete耗时。
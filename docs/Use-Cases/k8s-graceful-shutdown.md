...
1. Pod已被删除，但Ingress、Kube-proxy、外部LB控制器等还在分发新的流量。
...
2. Pod还未被删除，但Ingress或外部LB控制器等组件已经移除相关Pod。
...
3. Pod中的应用收到SIGTERM信号后，立即结束当前连接。导致尚未完成的业务传输被终止。
...
4. 超长活动连接在`terminationGracePeriodSeconds`到期后依然未结束，被kubelet强行Kill。
